...
export class ParticleRenderable extends Renderable<Particle[], ParticleRenderStyle> {
  ...
  private particleTrails: Map<string, {x: number, y: number, alpha: number, color: string}[]> = new Map();
...
//   }
  ...
  override render(ctx: OffscreenCanvasRenderingContext2D): void {
    ...
    // 调试信息：输出尾迹数量
    console.log(`尾迹数量: ${this.particleTrails.size}, 粒子数量: ${this.particles.length}`);
  }
  ...
  private emitParticle(): void {
    ...
    const emitterX = this.config.emitterX !== undefined ? this.config.emitterX : this.canvasWidth / 2;
    const emitterY = this.config.emitterY !== undefined ? this.config.emitterY : this.canvasHeight / 2;
