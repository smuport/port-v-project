...
export class GanttSvgRenderable extends Renderable<GanttTask[]> {
  ...
  renderSvg(svgRoot: SVGElement): SVGElement {
    ...
    const { startDate, columnWidth, rowHeight, headerHeight, colors } =
      this.config;
    ...
    this.tasks.forEach((task, index) => {
      ...
      if (task.progress > 0) {
        ...
        progressRect.setAttribute(
          'width',
          ((taskWidth * task.progress) / 100).toString()
        );
        ...
        progressRect.setAttribute(
          'fill',
          this.darkenColor(task.color || colors.taskDefault, 20)
        );
      ...
      // 创建任务名称文本
      const taskText = document.createElementNS(
        this.svgNamespace,
        'text'
      ) as SVGTextElement;
      ...
      if (task.dependencies && task.dependencies.length > 0) {
        task.dependencies.forEach((depId) => {
          ...
          if (depTask) {
            ...
            const depEndDays = this.getDaysDifference(
              startDate,
              depTask.endDate
            );
  ...
  private renderDateScale(svgRoot: SVGElement): void {
    const { startDate, endDate, columnWidth, headerHeight, colors } =
      this.config;
