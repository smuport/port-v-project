import { map, Observable, shareReplay, Subject, tap, withLatestFrom } from 'rxjs';
...
export class Layer<T = any, U = any> {
  ...
  eventObservable?: Observable<[callback: (evt: CpBaseEvent, data: T) => void, evt: CpBaseEvent, data: T]>;
  ...
  addEventListener(evtName: string, callback: (evt: CpBaseEvent, data: T) => void) {
  ...
  private checkRenderableSelection(renderable: Renderable<any>, selection: { x: number, y: number, w: number, h: number }, selectedData: any[]) {
