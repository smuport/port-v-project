...
export class YcRenderable extends Renderable<VisualEquipment,YcStyle> {
  ...
  override render(ctx: OffscreenCanvasRenderingContext2D): void {
    ...
    const { x, y, w, h, craneNumber, frameFillStyle, frameStrokeStyle, frameLineWidth, textStyle } = style;
    ...
    bottomBar.updateStyle({ ...frameStyle, x, y: y + h - frameHeight, w, h: frameHeight });
    ycName.updateStyle({ ...textStyle, x: x + w / 2, y: y + h / 2, text: craneNumber });
